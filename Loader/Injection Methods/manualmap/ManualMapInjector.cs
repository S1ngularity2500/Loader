    ...
    internal class ManualMapInjector {
        ...
        private bool InjectDependency(string dependency) {
            ...
            if (procAddress == IntPtr.Zero) {
                Trace.WriteLine("[InjectDependency] GetProcAddress failed");
                return false;
            }
            ...
            if (lpAddress == IntPtr.Zero) {
                Trace.WriteLine("[InjectDependency] RemoteAllocateMemory failed");
                return false;
            }
            ...
            if (result) {
                ...
                if (Imports.WaitForSingleObject(hHandle, TimeOut) != 0) {
                    Trace.WriteLine("[InjectDependency] remote thread not signaled");
                    return false;
                }
            }
            else
            {
                Trace.WriteLine("[InjectDependency] WriteProcessMemory failed");
            }
        ...
        private IntPtr GetRemoteModuleHandleA(string module) {
            ...
            if (dwStatus >= 0 && dwSize < dwSizeNeeded) {
                ...
                if (pbi == null) {
                    Trace.WriteLine("[GetRemoteModuleHandleA] Couldn't allocate heap buffer");
                ...
                if (imageImportDescriptor != null) {
                    for (; imageImportDescriptor.Value.Name > 0; imageImportDescriptor++) {
                        ...
                        if (moduleBase == IntPtr.Zero) {
                            ...
                            if (moduleBase == IntPtr.Zero) {
                                Trace.WriteLine("[ProcessImportTable] failed to obtain module handle");
                    ...
                    Trace.WriteLine("[ProcessImportTable] Size of table confirmed but pointer to data invalid");
                    // Size of table confirmed but pointer to data invalid
                    return false;
                }
            } else {
                Trace.WriteLine("[ProcessImportTable] no imports");
                // no imports
                return true;
            }
        }
                ...
                if (imageDelayedImportDescriptor != null) {
                    for (; imageDelayedImportDescriptor.Value.Name > 0; imageDelayedImportDescriptor++) {
                        ...
                        if (moduleBase == IntPtr.Zero) {
                            ...
                            if (moduleBase == IntPtr.Zero) {
                                Trace.WriteLine("[ProcessDelayedImportTable] no imports");
                                // failed to obtain module handle
                                continue;
                            }
                        }
                    ...
                    Trace.WriteLine("[ProcessDelayedImportTable] Size of table confirmed but pointer to data invalid");
                    // Size of table confirmed but pointer to data invalid
                    return false;
                }
            } else {
        ...
        private IntPtr LoadImageToMemory(IntPtr baseAddress) {
            ...
            if (imageNtHeaders == null) {
                Trace.WriteLine("[LoadImageToMemory] Invalid Image: No IMAGE_NT_HEADERS");
                // Invalid Image: No IMAGE_NT_HEADERS
                return IntPtr.Zero;
            }

            if (imageNtHeaders.Value.FileHeader.NumberOfSections == 0) {
                Trace.WriteLine("[LoadImageToMemory] Invalid Image: No Sections");
            ...
            if (imageNtHeaders.Value.OptionalHeader.ImageBase % 4096 != 0) {
                Trace.WriteLine("[LoadImageToMemory] Invalid Image: Not Page Aligned");
                return IntPtr.Zero;
            }

            if (imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.Size > 0) {
                Trace.WriteLine("[LoadImageToMemory] This method is not supported for Managed executables");
                return IntPtr.Zero;
            }

            var allocatedRemoteMemory = RemoteAllocateMemory(imageSize);
            if (allocatedRemoteMemory == IntPtr.Zero) {
                Trace.WriteLine("[LoadImageToMemory] Failed to allocate remote memory for module");
                return IntPtr.Zero;
            }

            if (!ProcessImportTable(baseAddress)) {
                Trace.WriteLine("[LoadImageToMemory] Failed to fix imports");
                return IntPtr.Zero;
            }

            if (!ProcessDelayedImportTable(baseAddress, allocatedRemoteMemory)) {
                Trace.WriteLine("[LoadImageToMemory] Failed to fix delayed imports");
                return IntPtr.Zero;
            }

            if (!ProcessRelocations(baseAddress, allocatedRemoteMemory)) {
                Trace.WriteLine("[LoadImageToMemory] Failed to process relocations");
                return IntPtr.Zero;
            }

            if (!ProcessSections(baseAddress, allocatedRemoteMemory)) {
                Trace.WriteLine("[LoadImageToMemory] Failed to process sections");
                return IntPtr.Zero;
            }

            if (!ProcessTlsEntries(baseAddress, allocatedRemoteMemory)) {
                Trace.WriteLine("[LoadImageToMemory] ProcessTlsEntries Failed");
                return IntPtr.Zero;
            }
            ...
            if (imageNtHeaders.Value.OptionalHeader.AddressOfEntryPoint > 0) {
                ...
                if (!CallEntryPoint(allocatedRemoteMemory, (uint)dllEntryPoint, AsyncInjection)) {
                    Trace.WriteLine("[LoadImageToMemory] Failed to call entrypoint");
                    return IntPtr.Zero;
                }
            }
